...
module.exports = (function () {
  ...
  if (!platform.isMQQBrowser() || !platform.isIOS()) {
    return function () {
      return Promise.reject(new Error('cannot execute ios bridge on an unavailable platform'));
    };
  }

  /**
   * source from http://jsapi.qq.com/get?api=app.getGeoLocation
   * <-- source start
   */
  ...
  */
  ...
  browser.define("browser.app.getGeoLocation", function(succCallback, errCallback){
    x5.exec(succCallback, errCallback || function(){}, "app", "getGeoLocation", []);
  });
    ...
    return new Promise(function (resolve, reject) {
      browser.app.getGeoLocation(function (position) {
        if (position.ret == 'true') {
          ...
          reject(new Error('cannot get geo-location by x5'));
        }
      }, reject);
    });
  };
